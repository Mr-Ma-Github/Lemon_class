...
import os
import time
#启动appiuml时,需要指定chromedriver.exe的目录。使用appium默认目录下的会报错。
#在切换到小程序webview时,会去匹配chrome内核的39的驱动。在切换完成之后,在打印所有的窗口
...
desired_caps["appPaqkage"] = "com.tencent.mm"
...
#所以为了避免这个问题,加上 androidProcess:com.tencent.mm:toolsm
...
print("当前所有的窗口为:",hs)
#print("当前所有的窗口为:", driver.current_window_handle)
#需要找到哪一个窗口有柠檬班信息的窗口,然后再在其下找元素操作。
#遍历所有的handles,找到当前页面所在的handle:如果pageSource有包含你想要的元素,就是所要找的handles
